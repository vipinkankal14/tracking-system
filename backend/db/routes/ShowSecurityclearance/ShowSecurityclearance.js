    ...
    try {
        ...
        const query = `
          SELECT 
            ...
            LEFT JOIN car_extended_warranty_requests ex
            ON c.customerId = ex.customerId
            AND adi.extended_warranty = 'Yes'
            AND (
                adi.auto_card = 'No'
                OR
                (adi.auto_card = 'Yes' AND aut.status = 'Approval' ) 
            )

            LEFT JOIN predeliveryinspection pre
            ON c.customerId = pre.customerId
            AND (
                -- Case 1: All additional services are declined
                (
                    adi.accessories = 'No' 
                    AND adi.coating = 'No' 
                    AND adi.rto = 'No' 
                    AND adi.fast_tag = 'No' 
                    AND adi.insurance = 'No' 
                    AND adi.auto_card = 'No'
                )
                OR
                -- Case 2: All active services are approved
                (
                    (adi.accessories = 'No' OR o.status = 'Approval') AND
                    (adi.coating = 'No' OR s.status = 'Approval') AND
                    (adi.rto = 'No' OR rt.status = 'Approval') AND
                    (adi.fast_tag = 'No' OR cir.status = 'Approval') AND
                    (adi.insurance = 'No' OR ins.status = 'Approval') AND
                    (adi.auto_card = 'No' OR aut.status = 'Approval')
                )
            )

            LEFT JOIN gate_pass gsc
            ON c.customerId = gsc.customerId
            AND (
                -- Case 1: All additional services are declined
                (
                    adi.accessories = 'No' 
           ...
           AND pre.status = 'Approval'  -- Added condition to check pre.status
            ...
            LEFT JOIN management_security_clearance mc
