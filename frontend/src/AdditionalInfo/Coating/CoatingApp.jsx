...
function CoatingApp() {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        if (data.success) {
          setCounts({
            Approval: data.data.counts.coating?.Approval || 0,
            Rejected: data.data.counts.coating?.Rejected || 0,
            Pending: data.data.counts.coating?.Pending || 0,
            totalInterested: data.data.counts.coating?.totalInterested || 0
          });
        } else {
  ...
  if (loading) {
    return (
      <Container>
        <LoadingContainer>Loading coating data...</LoadingContainer>
      </Container>
    );
  }
  ...
  if (error) {
    return (
      <Container>
        <ErrorContainer>
          Error loading coating data: {error}
          <br />
          <button 
  ...
  return (
    <Container>
      <Title>Coating Status Overview</Title>
       <Grid>
