...
const ExchangeApproved = () => {
    ...
    const fetchCarExchanges = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.data)) {
          // Filter only Approved exchanges
          const ApprovedExchanges = response.data.data.filter(
            (exchange) => exchange.status === "Approval"
          );
          setCarExchanges(ApprovedExchanges);
        } else {
  ...
  const handleRejectConfirmation = async () => {
    ...
    try {
      ...
      setError(
        `Failed to reject exchange: ${err.response?.data?.error || err.message}`
      );
    }
  };
  ...
  const getStatusColor = (status) => {
    return "success"; // All will be Approved in this version
  };
                        ...
                        <Box sx={{ mb: 2 }}>
                          ...
                          <Box sx={{ pl: 2 }}>
                            <Typography variant="body2">
                              <strong>Email:</strong> {exchange.email || "N/A"}
                            </Typography>
                            <Typography variant="body2">
                    ...
                    <Grid item xs={12} md={6}>
                      <Box sx={{ mb: 2 }}>
                        ...
                        <Box sx={{ pl: 2 }}>
                          ...
                          <Typography variant="body2" sx={{ mb: 1 }}>
                            <strong>Car Details Exchanges :</strong>{" "}
                            {exchange.carMake} | {exchange.carModel} |{" "}
                            {exchange.carColor}
                          </Typography>
                          <Typography variant="body2" sx={{ mb: 1 }}>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: "bold" }}>Ex-Car Owner</TableCell>
              <TableCell sx={{ fontWeight: "bold" }}>Car Details</TableCell>
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        <TextField
          variant="outlined"
          placeholder="Search..."
          label="Search Approved Exchanges"
          value={searchQuery}
      ...
      >
        <DialogTitle>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Typography>
              <strong>Reject Exchange for:</strong>{" "}
            ...
            {selectedExchange && (
              <>
                ...
                <Typography variant="body2" sx={{ mb: 1 }}>
                  ...
                  {new Date(selectedExchange.updatedAt).toLocaleString()}
                </Typography>
              </>
            )}
          </Box>
