...
const FinanceMobileCard = ({ customer, handleDetailsClick, handleReject }) => {
  const [expanded, setExpanded] = useState(false);
  const loan = customer.loans && customer.loans.length > 0 ? customer.loans[0] : null;

  return (
    <Card sx={{ mb: 2, border: `1px solid #4caf50` }}> {/* Approved status color */}
      <CardContent>
...
const FinanceTabletRow = ({ customer, handleDetailsClick, handleReject }) => {
...
const FinanceDesktopRow = ({ customer, handleDetailsClick, handleReject }) => {
  const [open, setOpen] = useState(false);
  const loan = customer.loans && customer.loans.length > 0 ? customer.loans[0] : null;
...
const FinanceDocumentsModal = ({ open, handleClose, selectedCustomer, selectedLoan }) => {
...
const RejectionModal = ({ open, handleClose, selectedCustomer, handleConfirmReject }) => {
...
const FinanceApproved = () => {
  ...
  // Filter customers based on search query and status (only Approved)
  const getFilteredFinanceCustomers = () => {
    return financeCustomers.filter(
      (customer) => 
        ...
        customer.loans?.some(loan => loan.status === "Approval")
    );
  };
  ...
  const handleFinanceConfirmReject = async (customer, reason) => {
    try {
      ...
      if (response.status === 200) {
        alert("Approved finance request has been rejected!");
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
      <Typography variant="h5" sx={{ mb: 3, color: "#071947", display: 'flex', alignItems: 'center' }}>
        <AccountBalanceIcon sx={{ mr: 1 }} /> Approved Finance Requests
      </Typography>
            ...
            <Box>
              {getFilteredFinanceCustomers().length > 0 ? (
                ...
                <Box sx={{ textAlign: 'center', my: 4 }}>
                  <Typography>No approved finance requests found.</Typography>
                </Box>
              )}
            </Box>
          )}
            ...
            <TableContainer component={Paper}>
              <Table size="small">
                ...
                <TableBody>
                  {getFilteredFinanceCustomers().length > 0 ? (
                    getFilteredFinanceCustomers().map((customer) => (
                      customer.loans.filter(loan => loan.status === "Approval").map((loan) => (
                        <FinanceTabletRow 
                    ...
                    <TableRow>
                      <TableCell colSpan={6} sx={{ textAlign: 'center' }}>
                        No approved finance requests found.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
