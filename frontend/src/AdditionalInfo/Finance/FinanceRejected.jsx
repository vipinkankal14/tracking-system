...
const FinanceMobileCard = ({ customer, handleDetailsClick, handleApprove }) => {
  ...
  return (
    <Card sx={{ mb: 2, border: `1px solid #f57c00` }}> {/* Rejected status color */}
      <CardContent>
            ...
            {loan && (
              <>
                ...
                <Typography variant="body2" sx={{ mt: 1, color: "red" }}>
                  <strong>Rejection Reason:</strong> {loan.financeReason}
                </Typography>
              </>
            )}
...
const FinanceTabletRow = ({ customer, handleDetailsClick, handleApprove }) => {
...
const FinanceDesktopRow = ({ customer, handleDetailsClick, handleApprove }) => {
  const [open, setOpen] = useState(false);
  const loan = customer.loans && customer.loans.length > 0 ? customer.loans[0] : null;
...
const FinanceDocumentsModal = ({ open, handleClose, selectedCustomer, selectedLoan, handleApprove }) => {
...
const FinanceRejected = () => {
  ...
  // Filter customers based on search query and status (only Rejected)
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
      <Typography variant="h5" sx={{ mb: 3, color: "#071947", display: 'flex', alignItems: 'center' }}>
        <AccountBalanceIcon sx={{ mr: 1 }} /> Rejected Finance Requests
      </Typography>
            ...
            <Box>
              {getFilteredFinanceCustomers().length > 0 ? (
                ...
                <Box sx={{ textAlign: 'center', my: 4 }}>
                  <Typography>No Rejected finance requests found.</Typography>
                </Box>
              )}
            </Box>
          )}
            ...
            <TableContainer component={Paper}>
              <Table size="small">
                ...
                <TableBody>
                  {getFilteredFinanceCustomers().length > 0 ? (
                    getFilteredFinanceCustomers().map((customer) => (
                      customer.loans.filter(loan => loan.status === "Rejected").map((loan) => (
                        <FinanceTabletRow 
                    ...
                    <TableRow>
                      <TableCell colSpan={6} sx={{ textAlign: 'center' }}>
                        No Rejected finance requests found.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
