  ...
  const handleApprove = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const newData = await axios.get("http://localhost:5000/api/showPreDeliveryInspection")
        setCustomers(newData.data.data)
      }
    } catch (err) {
      setError(`Failed to approve PDI: ${err.response?.data?.error || err.message}`)
      ...
      <Card sx={{ mb: 2, borderLeft: "4px solid #f44336" }}>
        <CardContent>
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={12}>
              <Chip label="Rejected" color="error" size="small" sx={{ mt: 1 }} />
            </Grid>
          </Grid>
        </CardContent>
      </Card>
