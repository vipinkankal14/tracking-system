...
export function ExchangeModal({ open, onClose, personalInfo }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!personalInfo?.customerId) {
      alert(
        "Please fill in your personal information before submitting the Car Exchange Services."
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/submitExchangeRequest",
        {
          method: "POST",
  ...
  return (
    <>
      ...
      <Dialog open={confirmationOpen}>
        ...
        <DialogContent>
          <DialogContentText>
            Your Exchange form has been submitted successfully.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
