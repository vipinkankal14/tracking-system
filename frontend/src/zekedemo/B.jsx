...
function PaymentHistory() {
  ...
  const [accountmanagement, setAccountmanagement] = useState(null);
  ...
  const handleRefundConfirmation = async () => {
    ...
    try {
      // Use POST to create a new refund record with status "Process"
      const response = await axios.post(
        `http://localhost:5000/api/refund/create`,
        {
          customerId, // Include customerId in the request body
          refundReason,
          refundAmount: parseFloat(refundAmount),
          // Status "Process" is hardcoded in the backend, so no need to send it
        }
      );
      ...
      setError(
        `Failed to initiate refund: ${err.response?.data?.error || err.message}`
      );
    }
  };
  ...
  const {
    status = "N/A",
    cancellationReason: statusCancellationReason = "N/A",
  } = accountmanagement || {};
      ...
      <div className="payment-history">
        <div className="header">
          ...
          {status && (
            ...
            >
              Status: <span style={{ color: "red" }}>{status || "N/A"}</span>
            </Typography>
          )}
          {status === "rejected" && (
            <Typography
            ...
            >
              ...
              <span style={{ color: "red" }}>
                {statusCancellationReason || "No reason provided"}
              </span>
            </Typography>
          )}
        ...
        >
          ...
          {/* Show Rejected button if status is not "rejected" */}
          {status !== "rejected" && (
            <Button
              size="small"
              variant="contained"
